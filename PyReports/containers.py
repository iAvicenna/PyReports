...
class _Container(objects._Context, objects._Node):
    ...
    def _items(self, item_name):
        ...
        for child in self._children:
            if isinstance(child, item_class):
                items.append(child)
                
            if isinstance(child, _Container):
                items += child.__getattribute__(f'_{item_name}s')
...
class Report(_Container):
    ...
    def to_html(self, output_path: str, return_html: bool=False):
        '''
    ...
    def _add_styles(self):
        ...
        code_style_css = f'    <link rel="stylesheet" href="{code_style_css_path}">\n'
...
class Tab(_Container):
    ...
    def _generate_html(self):
        ...
        for i in range(len(self._children)):
            ...
            tab_html += '    ' + indent + f'<button class="tablinks" {name}style="{button_styles[i]}" onclick="open_tab(event, \'{item_ids[i]}\',\'{tab_name}\' )">{self._tab_titles[i]}</button>\n'

        #open all button
        if len(self._children)>0:
            tab_html += '    ' + indent + f'<button class="tablinks" style="{button_styles[i]}" onclick="open_all_tabs(event, \'{tab_name}\')">Open All</button>\n'
    ...
    def _generate_html(self):
        ...
        for ind_child,child in enumerate(self._children):
            ...
            if self._item_titles is not None:
                child_html_str = self._item_titles[ind_child] + '<br>' + child_html_str
